...
######
def generate_filtered_gallery(iter_num, gallery_real_attribute, gallery_fake_features_attr):
    TP_list=[]
    FP_list=[]
    TN_list=[]
    FN_list=[]
    F1_list=[]
    remain_gallery_path_list_per_query_img_dict = {}
    ...
    if torch.cuda.is_available():
        ...
        print(f"disusable gpu, using cpu")
        for key, value in query_features_attr.items():
    ...
    # n
    def process_query(query_img_path):
        try:
            i = 0
            tp = tn = fp = fn = 0
            local_remain_filtered_gallery_features_dict = {}
            local_remain_gallery_path_list_per_query_img_dict = {}
            local_remain_gallery_path_list_per_query_img_dict[query_img_path] = []
            ...
            for gallery_img_path in gallery_img_path_list:        
                ...
                if(query_fake_up == gallery_fake_up and query_fake_down == gallery_fake_down):
                    local_remain_filtered_gallery_features_dict[gallery_img_path] = gallery_fake_features_attr[gallery_img_path]
                    local_remain_gallery_path_list_per_query_img_dict[query_img_path].append(gallery_img_path)
            ...
            i += 1
            end_time=time.time()
            print(f'resolution_{(iter_num+1)*10}_{os.path.basename(query_img_path)} 쿼리 결과 -','F1-score:', F1_score, f'fake_up:{query_fake_up}, fake_down:{query_fake_up}, time: {round(end_time-start_time,2)}초. {i + ( iter_num *len(query_img_path_list))}/{len(query_img_path_list) * 10}. ')
            ...
            return query_img_path, tp, fp, tn, fn, F1_score, local_remain_filtered_gallery_features_dict, local_remain_gallery_path_list_per_query_img_dict
        except Exception as e:
            print(f"Error processing {query_img_path}: {e}")
            exit()
            return 0, 0, 0, 0, 0, {}, {}
    num_cores = os.cpu_count()
    max_workers = num_cores if num_cores else 4  # 기본적으로 4로 설정

    query_img_path_list = list(query_img_path_list)[:10]  # 처음 10개 쿼리만 사용

    print(f"cpu logical core: {num_cores}")
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = [executor.submit(process_query, query_img_path) for query_img_path in query_img_path_list]

        for future in futures:
            query_img_path, tp, fp, tn, fn, F1_score, local_remain_filtered_gallery_features_dict, local_remain_gallery_path_list_per_query_img_dict = future.result()
            TP_list.append(tp)
            FP_list.append(fp)
            TN_list.append(tn)
            FN_list.append(fn)
            F1_list.append(F1_score)
            remain_filtered_gallery_features_dict.update(local_remain_filtered_gallery_features_dict)
            remain_gallery_path_list_per_query_img_dict[query_img_path] = local_remain_gallery_path_list_per_query_img_dict
    return TP_list, FP_list, TN_list, FN_list, F1_list, remain_filtered_gallery_features_dict, remain_gallery_path_list_per_query_img_dict
...
if __name__ == '__main__':
    ...
    for i in range(9, -1, -1):
        # remain_filtered_gallery_features_dict, key:  필터링 후 남은 갤러리 전체의 사진. value: [feature, attr]
        # remain_gallery_path_list_per_query_img_dict, key: query_img_path. value: remain_gallery_img_path
        TP_list, FP_list, TN_list, FN_list, F1_list, remain_filtered_gallery_features_dict, remain_gallery_path_list_per_query_img_dict = generate_filtered_gallery(i, gallery_real_attribute, gallery_fake_features_attr)

        # 각 쿼리 이미지별로 필터링 후 남은 이미지의 목록
        remain_gallery_path_list_per_query_img_dict_save_path = f"/root/amd/reid_model/datasets/Market-1501-v24.05.21_junk_false/filtering/filtered_gallery/meta/remain_gallery_path_list_per_query_img_dict_{(i+1) * 10}.pkl"
        save_data_as_pkl(remain_gallery_path_list_per_query_img_dict, remain_gallery_path_list_per_query_img_dict_save_path)

        # filtering 완료 후 남아있는 갤러리를 폴더에 복사함.
